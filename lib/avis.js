...
// Prévient le conseiller par email qu'un message l'attend. Best-effort : une
// panne d'email ne bloque jamais l'envoi du message. RGPD : le mail ne contient
// que le code pseudonyme, jamais l'état civil ni le contenu du message.
export async function notifyConseillerNewMessage({ conseillerUid, beneficiaireCode }) {
  if (!conseillerUid) return;
  try {
    const user = await adminAuth.getUser(conseillerUid);
    const to = user?.email;
    if (!to) return;

    const base = String(process.env.NEXT_PUBLIC_SITE_URL || '').replace(/\/+$/, '');
    const link = base ? `${base}/conseiller` : '/conseiller';
    const qui = beneficiaireCode ? `La personne ${beneficiaireCode}` : 'Une personne accompagnée';

    const text = [
      'Bonjour,',
      '',
      `${qui} vous a adressé un message sur IAMONJOB.`,
      '',
      `Vous pouvez le consulter et répondre depuis votre espace conseiller : ${link}`,
      '',
      "Par respect de la confidentialité, le contenu du message n'est pas repris dans cet email.",
      '',
      'IAMONJOB',
    ].join('\n');
    ...
    await sendEmail({
      to,
      subject: "IAMONJOB — nouveau message d'une personne accompagnée",
      text,
    });
  } catch (err) {
    // On ne journalise ni l'email ni le contenu : juste la cause technique.
    console.warn('[avis] Notification conseiller non envoyée :', err.message);
  }
}
...
// Lit un avis précis pour le conseiller qui le possède. Renvoie le document brut
// (avec context.shared et beneficiaireId), ou null si absent ou non détenu.
// Sert de capacité d'accès : posséder l'avis autorise à lire la fiche partagée.
export async function getAvisByIdForConseiller(avisId, conseillerUid) {
  const snap = await adminDb.doc(`${COL}/${avisId}`).get();
  if (!snap.exists) return null;
  const data = snap.data();
  if (data.conseillerUid !== conseillerUid) return null;
