...
async function callOnce({ prompt, systemInstruction, isJson, model, apiKey, timeoutMs, onUsage }) {
  const payload = {
    model,
    messages: [
  ...
  // Remontée du coût sur succès seulement (ce point n'est atteint qu'après un 200).
  if (typeof onUsage === 'function' && data.usage) {
    try { onUsage({ provider: 'mistral', model, tokens: data.usage.total_tokens ?? 0 }); } catch { /* ignore */ }
  }
...
export async function callMistral({
  ...
  for (let i = 0; i < attempts.length; i++) {
    try {
      return await callOnce({ prompt, systemInstruction, isJson, apiKey, ...attempts[i], onUsage });
    } catch (err) {
      lastErr = err;
