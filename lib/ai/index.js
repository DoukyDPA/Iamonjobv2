...
export async function callAI({
  ...
  // Mistral est le provider par défaut. Gemini reste disponible en secours
  // uniquement s'il est explicitement demandé.
  const providerKey = provider === 'gemini' ? 'gemini' : 'mistral';
  const model = MODELS[providerKey]?.[safeTask] || MODELS[providerKey].default;
  const fn = providerKey === 'mistral' ? callMistral : callGemini;

  const raw = await fn({ prompt, systemInstruction, isJson, model, onUsage });
...
export function availableProviders() {
  // Mistral en tête : c'est le provider par défaut, donc availableProviders()[0]
  // doit renvoyer 'mistral' quand la clé est présente.
  const providers = [];
