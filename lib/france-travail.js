...
export async function fetchFicheRome(codeRome) {
  ...
  // Timeout court (8 s) : la fiche n'est qu'un socle d'enrichissement. Si le
  // référentiel traîne, mieux vaut poursuivre sans elle que d'allonger le temps
  // de génération de l'enquête métier.
...
// Cache mémoire : nom de ville → code commune INSEE. Le référentiel bouge peu.
const communeCache = new Map();
const COMMUNE_TTL_MS = 24 * 60 * 60 * 1000; // 24 h

// Résout un nom de ville en code commune INSEE via l'API publique geo.api.gouv.fr
// (gratuite, sans authentification). Renvoie null si rien de fiable.
async function cityToInseeCode(name) {
  ...
  try {
    const url =
      `https://geo.api.gouv.fr/communes?nom=${encodeURIComponent(name)}` +
      `&fields=code&boost=population&limit=1`;
    const res = await fetchWithTimeout(url, { headers: { Accept: 'application/json' } }, 4000);
    if (!res.ok) return null;
    const data = await res.json();
    const code = Array.isArray(data) && data[0]?.code ? String(data[0].code) : null;
    communeCache.set(key, { code, expiresAt: Date.now() + COMMUNE_TTL_MS });
    return code;
  } catch {
    return null; // réseau/API indisponible : on retombe sur une recherche large.
  }
}

// Traduit une localisation en paramètres France Travail. L'API Offres attend un
// code commune INSEE (param `commune`) ou un numéro de département. Un nom de
// ville nu était ignoré jusqu'ici (résultats France entière) : on le résout donc
// en code INSEE. Codes postaux et départements gardent leur comportement.
async function resolveLocationParams(location) {
  if (!location) return {};
  const loc = String(location).trim();
  if (!loc) return {};

  if (/^\d{5}$/.test(loc)) return { commune: loc };
  if (/^(\d{2,3}|2[AB])$/i.test(loc)) return { departement: loc.toUpperCase() };

  // Nom de ville (ou de région) : on tente une résolution en code commune.
  const insee = await cityToInseeCode(loc);
  return insee ? { commune: insee } : {};
}
...
// `locParams` : objet déjà résolu ({ commune } ou { departement } ou {}).
async function fetchOffers(token, params, locParams, limit) {
  const query = new URLSearchParams({ sort: '1', range: `0-${limit - 1}`, ...params, ...locParams });
...
export async function searchOffers({ keywords, codeRome, location, limit = 5 }) {
  ...
  // Localisation résolue une seule fois (évite un appel géo par variante).
  const locParams = await resolveLocationParams(location);
  ...
  if (codeRome && /^[A-Z]\d{4}$/.test(codeRome.trim())) {
    try {
      const parRome = await fetchOffers(token, { codeROME: codeRome.trim() }, locParams, limit);
  ...
  if (keywords) {
    for (const variant of searchVariants(keywords)) {
      const resultats = await fetchOffers(token, { motsCles: variant }, locParams, limit);
