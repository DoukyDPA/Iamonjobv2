...
/**
 * Sauvegarde la liste des métiers mis de côté par la personne (favoris).
 * Chaque entrée : { title, codeRome }. Stockée dans le même document que le CV
 * pour être rechargée à la visite suivante.
 */
export async function saveFavoriteJobsToFirestore(userId, favoriteJobs) {
  try {
    await setDoc(
      doc(db, 'cvs', userId),
      { favoriteJobs, favoriteJobsUpdatedAt: serverTimestamp() },
      { merge: true }
    );
  } catch (err) {
    console.warn('[Firestore] Sauvegarde métiers favoris échouée :', err.message);
  }
}
