...
async function runAI(action, params) {
  const provider = availableProviders()[0] ?? 'mistral';
...
export async function generateCampaign({ uid, cvText, jobTitle, codeRome, companies }) {
  ...
  const companiesWithDrafts = companies.map((c) => ({
    ...
    stars:         c.stars         ?? null,
    lbbHasEmail:   c.lbbHasEmail   ?? false,
    // Contact résolu
    email:         c.email         ?? null,
 ...
 * L'appartenance à l'uid doit être vérifiée AVANT d'appeler cette fonction.
 *
 * @param {string} campaignId
 */
export async function deleteCampaign(campaignId) {
  await adminDb.doc(`campaigns/${campaignId}`).delete();
}
