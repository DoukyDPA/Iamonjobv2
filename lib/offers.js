...
// N'utilise une localisation texte que si ce n'est pas un code (postal/dépt) :
// Adzuna et Jooble attendent un nom de lieu, pas un code.
function textLocation(location) {
  if (!location) return '';
  const loc = location.trim();
  return /^\d+$/.test(loc) ? '' : loc.slice(0, 60);
}
...
async function searchJoobleOffers({ keywords, location, limit }) {
  ...
  try {
    ...
    if (!res.ok) {
      console.warn('[Offers/Jooble] Réponse', res.status);
      return [];
    }
    const data = await res.json();
