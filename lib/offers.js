...
// Nettoie un intitulé (appellation ROME) pour une recherche plein texte : retire
// les parenthèses, puis la seule variante féminine qui suit un slash tout en
// GARDANT la suite. « Chargé / Chargée de mission RSE » → « Chargé de mission
// RSE » (l'ancien split coupait à « Chargé » et perdait le mot porteur de sens).
function cleanTerm(s) {
  if (!s) return '';
  return s
    .replace(/\([^)]*\)/g, ' ')     // (H/F), (CDI)…
    .replace(/\s*\/\s*\S+/g, ' ')   // « / Chargée » retiré, « de mission RSE » conservé
    .replace(/\s+/g, ' ')
    .trim();
}

// ─── Pertinence : filtrer le bruit d'Adzuna et Jooble ────────────────────────
// Ces deux sources font une recherche plein texte « au sens large ». Sur « Chef
// de projet RSE », elles remontent tout ce qui contient « chef » (Médecin chef,
// chef de service…). On ne garde que les offres qui portent un mot VRAIMENT
// discriminant de la requête. France Travail n'est pas concerné : il cible par
// code ROME, donc précis par construction.

function norm(s) {
  return String(s || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
}

// Mots passe-partout : présents dans quantité d'intitulés, ils ne prouvent aucune
// pertinence. Un match sur « chef » ou « projet » seul ne veut rien dire.
const GENERIC_TERMS = new Set([
  'de', 'des', 'du', 'la', 'le', 'les', 'un', 'une', 'en', 'et', 'au', 'aux',
  'pour', 'sur', 'a', 'the', 'of', 'and', 'or',
  'h', 'f', 'hf', 'fh', 'cdi', 'cdd', 'stage', 'alternance', 'junior', 'senior',
  'chef', 'cheffe', 'responsable', 'agent', 'assistant', 'assistante',
  'charge', 'chargee', 'technicien', 'technicienne', 'operateur', 'operatrice',
  'employe', 'employee', 'manager', 'directeur', 'directrice', 'conseiller',
  'conseillere', 'cadre', 'specialiste', 'gestionnaire', 'coordinateur',
  'coordinatrice', 'projet', 'projets', 'mission', 'missions', 'service',
  'services', 'equipe', 'equipes', 'poste', 'metier',
]);

// Tokens qui portent le sens du métier recherché (ce qui reste une fois les mots
// passe-partout écartés). Ex. « Chef de projet RSE » → ['rse'].
function distinctiveTerms(keywords) {
  return Array.from(new Set(
    norm(keywords).split(/[^a-z0-9]+/).filter((t) => t.length >= 2 && !GENERIC_TERMS.has(t))
  ));
}

// Une offre est pertinente si son intitulé ou sa description contient au moins un
// token distinctif. Sans token distinctif (requête trop générique), on ne filtre
// pas : on n'aurait rien pour trancher.
function isRelevant(offer, terms) {
  if (!terms.length) return true;
  const hay = `${norm(offer.intitule)} ${norm(offer.description)}`;
...
export async function searchAllOffers({ keywords, codeRome, location, limit = 8 }) {
  ...
  // Filtre de pertinence : Adzuna et Jooble ratissent large et remontent du bruit
  // (« Chef de projet RSE » → Médecin chef). On ne garde que les offres portant un
  // mot distinctif de la requête. France Travail cible par code ROME : on le garde
  // tel quel, sans risque de perdre une offre valable dont le titre varie.
  const terms = distinctiveTerms(keywords);
  const keep = (list) => list.filter((o) => isRelevant(o, terms));
  ...
  const lists = [val(ft), keep(val(adzuna)), keep(val(jooble))];
