...
export async function resolveContact(company) {
  // On préserve toutes les métadonnées LBB (nafText, city, headcountText…)
  // pour qu'elles survivent jusqu'au document de campagne.
  const base = {
    siret:         company.siret         ?? '',
    name:          company.name          ?? '',
    naf:           company.naf           ?? '',
    ...
    headcountText: company.headcountText ?? '',
    stars:         company.stars         ?? null,
    lbbHasEmail:   company.lbbHasEmail   ?? false,
    email:         null,
    candidates:    [],
    domain:        null,
    method:        'no_domain',
    lbbUrl:        company.url ?? company.lbbUrl ?? null,
  };
